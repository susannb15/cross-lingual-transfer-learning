...
with open("probing_de_on_de.csv", "w+", encoding='utf-8') as f:
	f.write("layer,checkpoint,score")

for i in range(13):
	clf = os.path.join("classifiers", "clf_"+str(i))
	for j in a:
		rep_path = os.path.join("representations_de", str(j))
		reps = os.path.join(rep_path, "representations_"+str(i))
		prob = Probing(clf, reps)
		score = prob._predict()
		with open("probing_de_on_de.csv", "a", encoding='utf-8') as f:
